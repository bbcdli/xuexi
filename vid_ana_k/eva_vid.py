...
v_dirs = sorted([s for s in os.listdir(TEST_VIDEO_LOAD_PATH) if '.mp4' in s
                 and ('04_Tschetschenischer_Tuersteher_185_202_T' in s)]) #hyhy
#04_Tschetschenischer_Tuersteher_T_10_22
#01_fight_in_train_70_99_T
#02_Tschetschene_vs_Russe_T
#04_Tweety im Zug_29_49_F
#03_Tweety im Zug119_132_F
#02_Tweety im Zug105_115_F
#Latin Kings vs Tango Blast Houstone
#Hooligans_En_de_Hools_M_Old_School_0628_end_T
#04_Tschetschenischer_Tuersteher
#2017-09-06_13.57.41.5.cam_55_4.event57_testF
#Dog_lover_knocks_out_a_dog_abuser_0614_0637.mp4
...
def eva_one_clip(X, vid_view,start_frame, model, EVA_SAVE_PATH_NO_AGGR,
 ...
 pred_txt = gt_txt if top_inds[0] == gt_label else 'wait'
 p = max(output[0])
  ...
  while True:
   ...
   if ret:
    ...
    if frame_i % 1 == 0:
     ...
     if frame_i % CLIP_LENGTH == 0 and frame_i>0:
      print 'frame:',frame_i
...
##############################
LOG_ON = True
PROJ_DIR = '/home/hy/Documents/hy_dev/aggr/c3d-keras/'
PROJ_DIR = os.path.dirname(os.path.abspath(__file__))
NUM_CLASSES = 2
#os.path.dirname(os.path.abspath(__file__))
# TEST_V_PATH = PROJ_DIR + 'test_videos/'
TEST_VIDEO_LOAD_PATH = '/home/hy/Documents/hy_dev/aggr/aggr_vids/'
EVA_SAVE_PATH_NO_AGGR = os.path.join(PROJ_DIR,'save_no_aggr/')
if not os.path.exists(EVA_SAVE_PATH_NO_AGGR):
	os.makedirs(EVA_SAVE_PATH_NO_AGGR)
EVA_SAVE_PATH = os.path.join(PROJ_DIR,'save_aggr/')
...
#Dog_lover_knocks_out_a_dog_abuser_0614_0637.mp4
#02_Tweety im Zug105_115_F 03_Tweety im Zug119_132_F 01_fight_in_train_T
#02_Tschetschene_vs_Russe_T
# v_dirs = sorted([s for s in os.listdir(TEST_VIDEO_LOAD_PATH) if '.mp4' in s and ('2017-09-06_13.57.41.2.cam_55_3.event48T' in s)])
TEST_VIDEOS = []
for dir in v_dirs:
...
def main(model_name):
	...
	if backend == 'th':
		model_weight_filename = os.path.join(model_dir, model_name)
		model_json_filename = os.path.join(model_dir, 'sports1M_model_custom.json')
	else:
		model_weight_filename = os.path.join(model_dir, model_name)
		model_json_filename = os.path.join(model_dir, 'sports1M_model_custom.json')
	print("[Info] Reading model architecture...")
			...
			for num in xrange(total_test_clips):
				...
				def eva_one_clip(X, start_frame, model, EVA_SAVE_PATH_NO_AGGR, EVA_SAVE_PATH,count_correct):
					...
					if save_probability_to_png:
						...
						plt.savefig('probabilities_'+v_str+'.png')
						print('Maximum probability: {:.5f}'.format(max(output[0])))
						print('Predicted label: {}'.format(labels[output[0].argmax()]))
					# sort top five predictions from softmax output
					top_inds = output[0].argsort()[::-1][:5]  # reverse sort and take five largest items
					top_inds_list = top_inds.tolist()
					print('\nTop probabilities and labels:') #array([0, 1])
					for i,index in zip(top_inds_list,xrange(NUM_CLASSES)): #[1,0] [0,1]
						if index == 0:
							pred = int(output[0][i])
							if i == gt_label:
								count_correct += 1
								print('{1}: {0:.5f}  top1'.format(pred, labels_txt[i])),'correct',count_correct
							else:
								print('{1}: {0:.5f}  top1'.format(pred, labels_txt[i]))
						else:
							print('{1}: {0:.5f}  other'.format(int(output[0][i]), labels_txt[i]))
...
if __name__ == '__main__':
  model_name = 'k_01-0.62_0.27.hdf5'
  main(model_name)
