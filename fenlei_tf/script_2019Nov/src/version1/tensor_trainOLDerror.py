...
############################################################
RETRAIN          = True
current_step     = 10591 #4311, 4791,1211, 3271, 3491, 21291 72.4 model_60_h18_w18_c8-79302-top
step = 0
...
#dropout = 0.3
dropout = [0.50, 0.50, 0.5, 0.5] #3,4,5,5
...
#trained_model = "./testbench/" + "model_GD720_h184_w184_c6_3conv_L0.75_R0.7_U1.0_V1.0_31_0.82-6481.meta"
trained_model = "./logs/" + "model_GD360_h184_w184_c6all_5_0.49-10591.meta"
#model_GD720_h184_w184_c6_3conv_L0.75_R0.7_U1.0_V1.0_31_0.82-6481
...
training_iters = 151 #30000 # 1500  12500,
if CONTINUE_TRAIN:
...
def EVALUATE_IMAGES_sort_activation(sess):
  ...
  for i in range(0, TEST_length, 1):
    ...
    output = sess.run(pred, feed_dict={x: batch_xs, y: batch_ys, keep_prob: dropout_1s})
...
def EVALUATE_IMAGES_VAGUE():
  ...
  for i in range(0, TEST_length, 1):
    ...
    slices_rec = prep_images.create_test_slices(test_image,patch_size,test_image_label)
    ...
    #output_im = sess.run(pred, feed_dict={x: batch_xs, y: batch_ys, keep_prob: dropout_1s})
...
def EVALUATE_IMAGE_SLICES(img,f,index,sess, cartargets): #hy todo change dimension to fit tensorflow
  ...
  output = sess.run(pred, feed_dict={x: batch_xs1, y: batch_ys1, keep_prob: dropout_1s})
...
def confusion_matrix(labels_onehot, scores, normalized=True):
  ...
  print '\nn_samples for validation:', n_samples
...
################################################ Graph begin
  ...
  if optimizer_type == 'RMSProp':
    # hy: GradientDescentOptimizer
    optimizer = tf.train.RMSPropOptimizer(learning_rate=learning_rate, decay=decay, momentum=momentum, epsilon=epsilon_R, use_locking=False, name='RMSProp')
...
if RETRAIN or CONTINUE_TRAIN:
  ...
  with tf.Session() as sess:
    ...
    if CONTINUE_TRAIN: #set model path
      ...
      #ckpt = tf.train.get_checkpoint_state(checkpoint_dir="/home/adas/."c) #for running on the server
      if ckpt and ckpt.model_checkpoint_path:
    ...
    while step < training_iters and not set_STOP:
      for batch_step in xrange(int(train_size / batch_size)):
        ...
        try:
          ...
          train_acc = sess.run(accuracy, feed_dict={x: batch_xs, y: batch_ys, keep_prob: dropout})
          loss = sess.run(cost, feed_dict={x: batch_xs, y: batch_ys, keep_prob: dropout})
          #learning_rate = tf.train.exponential_decay(learning_rate, step, 10000, 0.96, staircase=True)
          #optimizer = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(cost)
        ...
        if step % display_step == 0:
          ...
          if show_test_acc == 1:
            val2_acc,val2_loss,val2_output = sess.run([accuracy,cost,pred], feed_dict={x: val2_batch_xs,
                                                                                       y: val2_batch_ys, keep_prob: dropout_1s})

          print "Validation accuracy=", "{:.4f}".format(val1_acc),
          if show_test_acc == 1:
            print "test accuracy=", "{:.4f}".format(val2_acc)
          ...
          confMat, confMat2 = confusion_matrix(val1_batch_ys, val1_output, normalized=True)
          ...
          if show_test_acc == 1:
            confMat, confMat2 = confusion_matrix(val2_batch_ys, val2_output, normalized=True)
          ...
          if save_all_model == 1:
            if show_test_acc == 1:
              ...
              saver.save(sess, save_path=model_path_str + 'all_' + str(batch_step) + '_' + str(round(val1_acc,2)), global_step=step)  # hy: added. It saves both all variables and GRAPH
          ...
          if show_test_acc == 1:
            ...
            if val2_acc > act_min and val2_acc < act_max and imgNum < 2.3 * settings.maxNumSaveFiles:  # hy: activate random rotation
              ...
              if imgNum > 2 * settings.maxNumSaveFiles:
                prep_images.REMOVE_online_Data(step)
              prep_images.rotateflipImg(rotation_angle, 0, noise_level, step)  # hy: angle,flipX,noise_level,step
              add_data = 1
...
if TEST_with_Images:
  ...
  with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
      ...
      confMat1_TEST_i, count_labels, confMat3, class_probability,overall_acc,mAP = EVALUATE_IMAGES(sess,6, LABEL_LIST_TEST, LABELS)
    ...
    trained_model_short = os.path.basename(trained_model)[:-5]

    for filename in os.listdir("."):
      if filename.startswith("eva-" + trained_model_short + '.txt'):
        os.rename(filename, 'eva-' + trained_model_short + '-' + str(overall_acc) + '-mAP' + str(mAP) + '.txt')
    print '\nsaving overall acc to file name'  # (eva)
