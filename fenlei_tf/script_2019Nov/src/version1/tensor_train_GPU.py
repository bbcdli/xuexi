...
#hy:Changes by Haiyan, 7Dec.2016 v0.41
...
############################################################
RETRAIN          = True
current_step     = 6411 #4311, 4791,1211, 3271, 3491, 21291 72.4 model_60_h18_w18_c8-79302-top

# Network Parameters
learning_rate = 0.008098 #0.04049 #0.03049 #0.015 #0.07297 #0.09568# TODO 0.05  0.005 better, 0.001 good \0.02, 0.13799 to 0.14 good for 6 classes,
# #0.13999 (flat) to 0.13999  (gap) for 7 classes, 0.0699965 for 6 classes with fine samples
#0.0035 for links+rechts 98%;
dropout = 0.5 # Dropout, probability to keep units

n_hidden = 720#360  #162*6 # 128
#300: horizontal 20%
...
SAVE_Misclassified     = 1
SAVE_CorrectClassified = 1
...
if CONTINUE_TRAIN:
  training_iters = current_step + 90000010
...
def EVALUATE_IMAGES_sort_activation(sess):
  ...
  for i in range(0, TEST_length, 1):
    ...
    print 'Ground truth label:', LABELS[label_target][:-1], ',  predict:', LABELS[RES][:-1],', pres:',output[0][RES]  # hy
    ...
    rank_index(output[0], label_target)
...
def EVALUATE_IMAGES_VAGUE():
  ...
  for i in range(0, TEST_length, 1):
    ...
    output_im = rank_index(outbox[0],test_image_label)
...
def EVALUATE_IMAGE_SLICES(img,f,index,sess, cartargets): #hy todo change dimension to fit tensorflow
  ...
  #print '\nRank list of predicted results'
  sorted_vec = rank_index(output[0], label_target)
...
if Graph_3conv == 1:
  ...
  def conv_net(_X, _weights, _biases, _dropout):
    ...
    print '- dropout ( keep rate', dropout, ')', conv1.get_shape()
    ...
    print '- dropout ( keep rate', dropout, ')', conv2.get_shape()
    ...
    print '- dropout ( keep rate', dropout, ')', conv3.get_shape()
    ...
    print '- dropout ( keep rate', dropout, ')', dense1.get_shape()
  ...
  tf.image_summary('Original', tf.reshape(x, shape=[-1, settings.h_resize, settings.w_resize, 1]),
                   max_images=1)  # hy:images_view

  # images after conv1 before max pool
  # _X = tf.reshape(x, shape=[-1, 24, 42, 1])
  _X = tf.reshape(x, shape=[-1, settings.h_resize, settings.w_resize, 1])  # hy for display
  ...
  tf.image_summary('2.Conv', tf.reshape(conv2, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=conv2_out)  # hy

  # hy: conv3 view
  conv3 = conv2d(conv2, weights['wc3'], biases['bc3'], 1)
  conv3 = tf.add(conv3, 0, name="conv3")
  print 'for conv3 view', conv3.get_shape()
  tf.image_summary('3.Conv', tf.reshape(conv3, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=conv3_out)  # hy
...
################################################ Graph 4conv begin
  ...
  def conv_net(_X, _weights, _biases, _dropout):
    ...
    print '- dropout ( keep rate', dropout, ')', conv4.get_shape()
  ...
  weights = {
    'wc1': tf.Variable(tf.random_normal([filter_size_1, filter_size_1, 1,      SEED], stddev=0.1, seed=SEED),name="wc1"),  # 5x5 conv, 1 input, 8 outputs
    'wc2': tf.Variable(tf.random_normal([filter_size_2, filter_size_2, SEED, conv2_out], stddev=0.1, seed=SEED),name="wc2"),  # 5x5 conv, 8 inputs, 16 outputs
    #hy:added new layers
    'wc3': tf.Variable(tf.random_normal([filter_size_3,filter_size_3,conv2_out,conv3_out],stddev=0.1,seed=SEED),name="wc3"),
    'wc4': tf.Variable(tf.random_normal([filter_size_4,filter_size_4,conv3_out,conv4_out],stddev=0.1,seed=SEED),name="wc4"),
    #'wc5': tf.Variable(tf.random_normal([filter_size_5,filter_size_5,conv4_out,conv5_out],stddev=0.1,seed=SEED),name="wc5"),

    #'wd1': tf.Variable(tf.random_normal([16 * 24 / 2 * 42 / 2, n_hidden], stddev=0.1, seed=SEED)),  # fully connected, 8*8*64 inputs, 1024 outputs
    # 'wd1': tf.Variable(tf.random_normal([8 * 8 * 64, 1024], stddev=0.1)),  # fully connected, 8*8*64 inputs, 1024 outputs
    'wd1': tf.Variable(tf.random_normal([6*6*conv4_out, n_hidden], stddev=0.1, seed=SEED),name="wd1"), #hy: fully connected, 8*8*64 inputs, 1024 outputs
    ...
    'out': tf.Variable(tf.random_normal([n_hidden, n_classes], stddev=0.1, seed=SEED),name="w_out")  # 1024 inputs, 10 outputs (class prediction)
  }
  ...
  if optimizer_type == 'adam':
    #hy: Adam with these parameters beta1=0.9,beta2=0.999, epsilon=1e-08 etc the training accuracy is not stable, epsilon = 0.01 better for these data
    print '\noptimizer:', optimizer_type, 'learning_rate:', learning_rate, '\nbeta11:', beta1, '\tbeta2:', beta2, '\tepsilon:', epsilon
    optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=beta1, beta2=beta2, epsilon=epsilon, use_locking=False, name='Adam').minimize(cost)
  ...
  tf.image_summary('1.Conv', tf.reshape(conv1, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=4,name="conv1-valid") #hy
  ...
  tf.image_summary('2.Conv', tf.reshape(conv2, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=4,name="conv2-valid") #hy
  ...
  tf.image_summary('3.Conv', tf.reshape(conv3, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=4,name="conv3-valid") #hy
  ...
  tf.image_summary('4.Conv', tf.reshape(conv4, shape=[-1, conv_view_size, conv_view_size, 1]), max_images=4,name="conv4-valid") #hy
...
#use ipython
#img = np.float32(PIL.Image.open('./Data/kache_rz82_d8_8.jpg'))
#showarray(img)
    ...
    while step < training_iters and not set_STOP:
      ...
      for batch_step in xrange(int(train_size / batch_size)):
        ...
        if step % display_step == 0:
          ...
          print "\nIter " + str(step) + ", Minibatch Loss= " + "{:.6f}".format(loss) + ", Training Accuracy= " \
                + "{:.4f}".format(train_acc)
          ...
          ## Validation  ####################################################################
          val1_acc = sess.run(accuracy, feed_dict={x: val1_batch_xs, y: val1_batch_ys, keep_prob: 1.})

          val2_acc = sess.run(accuracy, feed_dict={x: val2_batch_xs, y: val2_batch_ys, keep_prob: 1.})
          val2_loss = sess.run(cost, feed_dict={x: val2_batch_xs, y: val2_batch_ys, keep_prob: 1.})

          print "Validation accuracy=", "{:.4f}".format(val1_acc), ','  "test accuracy=", "{:.4f}".format(val2_acc)

          #print("Argmax of y:", targetindex)
          output = sess.run(pred, feed_dict={x: val2_batch_xs, y: val2_batch_ys, keep_prob: 1.})
          ...
          #hy: added to display all results in one graph
          train_res = sess.run(summary_op, feed_dict={x: batch_xs, y: batch_ys, keep_prob: 1})
          train_writer.add_summary(train_res, step)

          val1_res = sess.run(summary_op, feed_dict={x: val1_batch_xs, y: val1_batch_ys, keep_prob: 1})
          validation_writer.add_summary(val1_res, step)

          val2_res = sess.run(summary_op, feed_dict={x: val2_batch_xs, y: val2_batch_ys, keep_prob: 1})
          ...
          if save_all_model == 1:
            saver.save(sess, save_path=model_path_str + 'all', global_step=step)  # hy: added. It saves both all variables and GRAPH
          ...
          if (num_of_classified_classes > 1) or loss < last_loss or val2_acc > last_best_test_acc:
            ...
            if num_of_classified_classes > 0 and sum_col_min < 1.1 and val2_acc > last_best_test_acc-0.001 \
                    and loss < 1 and val2_acc > 0.7 and loss < 0.09:
          ...
          if val2_acc > 0.2 and (float(val2_loss / loss) > stop_loss
            ...
            if val2_acc > act_min and val2_acc < act_max and imgNum < 2.3 * settings.maxNumSaveFiles:  # hy: activate random rotation
              ...
              if imgNum > 2 * settings.maxNumSaveFiles:
                image_distortions.REMOVE_online_Data(step)
              image_distortions.rotateflipImg(rotation_angle, 0, noise_level, step)  # hy: angle,flipX,noise_level,step
              add_data = 1
              # training_size = int(total_images * TrainingProp)
              # batch_xs, batch_ys = digits.images[0:training_size], digits.target[0:training_size]
...
if TEST_with_Video:
  with tf.Session() as sess:
    ...
    for video_index in xrange(len(video_list)):
      #TestFace = settings.LABELS[0][:-1] # only one 'vorn'  #'hinten' # full, 0 hinten, 1 links, 2 oben, 3 rechts, 4 unten, 5 vorn,
    ...
    #VIDEO_FILE, crop_x1, crop_y1, area_step_size, video_label = set_TestClass(TestFace, video_window_scale)
