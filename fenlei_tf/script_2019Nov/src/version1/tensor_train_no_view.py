  ...
  while step < training_iters and not set_STOP:
   ...
   for batch_step in xrange(int(train_size / batch_size)):
    ...
    if step % display_step == 0:
     ...
     #output = sess.run(pred, feed_dict={x: val1_batch_xs, y: val1_batch_ys, keep_prob: dropout_1s})
     ...
     #confMat, confMat2 = confusion_matrix(val1_batch_ys, output, normalized=True)
     ...
     if save_all_models == 1:
      #saver.save(sess, save_path=model_path_str + 'all_' + str(batch_step) + '_' + str(round(val1_acc, 2)),
      saver.save(sess, save_path=model_path_str + 'all_' + str(batch_step),
