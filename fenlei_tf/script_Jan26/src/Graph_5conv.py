...
####################################################### Header begin ###################p#############################
###############################
dropout = [0.95, 0.95, 0.5, 0.5, 1, 1] #3,4,5,5
dropout_1s = [1]*len(dropout)
n_hidden = 128*4  #162*6 # 128
...
######################
#GD
learning_rate = 0.00419 # 0.00405 good #0.09 bad 3549 #0.04049 #0.03049 #0.015 #0.07297 #0.09568# TODO 0.05  0.005 better, 0.001 good \0.02, 0.13799 to 0.14 good for 6 classes,
######################
#adam
beta1 = 0.9
beta2 = 0.999
...
def define_model():
    ...
    def conv_net(_X, _weights, _biases, _dropout):
        ...
        print '\nconv5 ( f=', filter_size_5, 'k=', k, ')', conv5.get_shape()

        k = 2
        conv5 = max_pool(conv5,k)
        print 'conv5 max pooling (k=',k,')', conv5.get_shape()
        conv5 = tf.nn.dropout(conv5,_dropout[4])
        print '- dropout ( keep rate', dropout[4], ')', conv5.get_shape(), '<--'
    ...
    # 5x5 conv, 1 input, 8 outputs
        ...
        'wd1': tf.Variable(tf.random_normal([2 * 2 * conv5_out, n_hidden], stddev=0.1, seed=SEED), name="wd1"),
    ...
    biases = {
        ...
        'bc5': tf.Variable(tf.random_normal([conv5_out]), name="bc5"),
