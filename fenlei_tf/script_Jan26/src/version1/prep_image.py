  ...
  if os.path.isfile(crop_source_path + item):
   ...
   if crop_method == 'seq':
    ...
    for x in xrange(w):
     ...
     for y in xrange(h):
      ...
      #crop from center left
      #bbox = (int(x+w/17) * stride, y * stride, int(x+w/17) * stride + patch_size, y * stride + patch_size_h)
    ...
    for x in xrange(w):
     ...
     for y in xrange(h):
      if (y*stride + patch_size) > h or num_of_samples > crop_num:
       break
       #(48-1*1-42=5,48-1*1-42=5, 48-1*1=47, 48 - 1 * 1=47)
      #crop from right
      bbox = (w - x * stride - patch_size, h - y * stride - patch_size_h, w - x * stride, h - y * stride)

      #crop from center right
      #bbox = (w - int(x+w/18) * stride - patch_size, h - y * stride - patch_size_h, w - int(x+w/18) * stride, h - y * stride)
      bboxes_u.append(bbox)
      slice = im.crop(bbox).save(crop_dest_path + fname_wo_end +'_u'+ str(w-x*stride) +'_'+str(h-y*stride)+
               str(patch_size)+ImageType, optimize=True,bits=6)
      num_of_samples = len(bboxes_u) + len(bboxes_d)
   ...
   if crop_method == 'random':
    ...
    for imgN in xrange(crop_num):
     ...
     crop_dest_path + fname_wo_end + str(imgN) + '_' + str(stride) + '_l' + str(x * stride) + '_'+ str(y * stride) + ImageType, optimize=True, bits=6)
    ...
    for imgM in xrange(crop_num):
     ...
     slice = im.crop(bbox).save(
     crop_dest_path + fname_wo_end + str(imgM) + '_' + str(stride) + '_r' + str(x * stride) + '_' + str(y * stride) + ImageType, optimize=True, bits=6)
...
if prepare_active_fields == 1:
 ...
 crop_none_overlap(crop2_source_path=tmp_path, crop2_dest_path=crop_dest_path, crop_opt='equal',
                   num_of_parts_per_row=4)
...
def add_background_to_CAD():  # add_background
 ...
 crop_dest_path = '../tmp/resized/rest4/'  # MUST use label folder here
...
if split_Data == 1:
 ...
 for i in xrange(length):
  # cmd = 'mv ' + '../Data/data_3/cam2/' + fileprefix[i] + '*.*' + ' ' + '../Data/data_3/' + folder_list[i]  # hy: move
  cmd = 'mv ' + settings.data + fileprefix[i] + '*.*' + ' ' + settings.data + settings.LABELS[i]  # hy: move
  os.system(cmd)

if split_Inputpatches == 1:
 print 'splitting input patches'
 for i in xrange(length):
  cmd = 'mv ' + settings.patches + fileprefix[i] + '*.* ' + settings.patches + settings.LABELS[i]  # hy: move
  os.system(cmd)

if split_Test_Images == 1:
 print 'splitting test images'
 for i in xrange(length):
  cmd = 'mv ' + settings.test_images + fileprefix[i] + '*.* ' + settings.test_images + settings.LABELS[i]  # hy: move
  os.system(cmd)
...
if move_from_tmp2_to_patches == 1:
 print 'moving some data from tmp to patches'
 for i in xrange(length):
  # cmd = 'mv ' + tmp2 + fileprefix[i] + '*.* ' + test_images + folder_list[i]  # hy: cp to
  cmd = 'mv ' + settings.tmp2 + fileprefix[i] + '*.* ' + settings.patches + settings.LABELS[i]  # hy: cp to
  os.system(cmd)

if move_from_tmp2_to_testImg == 1:
 print 'moving from tmp2 to test images sub-folders'
 for i in xrange(length):
  # cmd = 'mv ' + data + folder_list[i]  + '/' + fileprefix[i] + '*.* ' + patches + folder_list[i]  # hy: move to
  cmd = 'mv ' + settings.tmp2 + fileprefix[i] + '*.jpg ' + settings.test_images + settings.LABELS[i]  # hy: move to
  os.system(cmd)
...
if copy_from_data_to_patches == 1:
 ...
 for i in xrange(length):
  ...
  # cmd = 'cp -r ' + patches + folder_list[i] + '/' + fileprefix[i] + '*.jpg ' + data + folder_list[i]  # hy: cp to
  os.system(cmd)
...
if copy_from_misclassifed_to_data == 1:
 ...
 cmd = 'cp -r ' + settings.misclassifed + '*.* ' + settings.data  # hy: cp to data
 os.system(cmd)
