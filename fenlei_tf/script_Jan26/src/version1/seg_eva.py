...
bg_model = 'weights_no_full_432.hdf5'  #11 80 432
...
def ROI(obj_area, im_crop, w, h, im_i=0, save_file=False):  # obj_area size 320x320 #(roi)
 ...
 def find_contour_mul(obj_area):
  contours, screen_out = find_contour(obj_area, 100)
  if not screen_out:
...
def do_classification(frame,video_frame_i,frame_crop_color,confMat2_TEST_Video, crop_y1, crop_y2,
...
def do_segment(model,im_crop_color,im_i,h,w):
  ####### convert into the shape for seg model input
  print w,h
  w=320
  h=320
  im_crop = im_crop_color.copy()
...
def EVALUATE_VIDEO_seg_test_OK(sess, VIDEO_FILE, num_class):  # (v)
 ...
 while True :#and video_frame_i < 850:
  ...
  if ret:
   ...
   '''
   tensorImgIn = frame_crop
   cv2.namedWindow('tensorImgIn', cv2.WINDOW_NORMAL)
   cv2.putText(frame_crop, 'frame_ts ' + str(video_frame_i), org=(320 / 10, 320 / 8),
               fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
               color=(0, 255, 0), thickness=2)
   #cv2.imshow('tensorImgIn', frame_crop)
   ...
   im = frame_crop_resize_gray
   im = np.asarray(im, np.float32)
   cv2.imshow("TensorFlow Window", imutils.resize(im.astype(np.uint8), 184))
   ...
   # send to classifier model
   start_cl = time.time()
   #classifier_res = classify_image(sess, video, test_image, test_labels, video_frame_i, frame_ori, frame_crop,
   #                                confMat2_TEST_Video, crop_y1, crop_y2, crop_x1, crop_x2)
   
   classify_image(sess, video, test_image, test_labels, video_frame_i, frame, frame_crop,
                  confMat2_TEST_Video, crop_y1, crop_y2, crop_x1, crop_x2)
   
   end_cl = time.time()
   print 'time elapsed for classification', end_cl - start_cl  # 0.0430190563202
   ######################### get classified result start#################################
   ######################### Tensorflow
   '''
   batch_xs, batch_ys = test_image, test_labels
   ...
   cv2.putText(frame, "predicted: " + label_pred_str, org=(frame.shape[1] / 3, frame.shape[0] / 10),
   ...
   # cv2.putText(frame, "prob:" + prob_str, org=(w_frame / 10, h_frame / 8),
   #            fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(0, 255, 0), thickness=1)
   cv2.putText(frame, 'frame ' + str(video_frame_i) + ' acc:' + prob_str, org=(20, 50),
               fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
               color=(0, 255, 0), thickness=2)
   #cv2.waitKey(20)
   #return frame
   ...
   ######################### get classified result end ##################################


   frame = classifier_res
   frame_demo = imutils.resize(frame, width=800)
...
if EVALUATE_WEBCAM:
 with tf.Session() as sess:
  ...
  n_classes = 6
  EVALUATE_WEBCAM_seg_and_classify(camera_port, False, n_classes)
