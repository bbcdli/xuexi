...
do_multiple_test = True  # this can only be started with prepared shell script eva.sh
############################################################
if do_multiple_test:
	# use setup in eva.sh
	classifier_model = sys.argv[1] + ".meta"
	# received format ./testbench/6classes/1/model_GD360_h184_w184_c6all_16_tr_0.57-121.meta
	classifier_model = settings.absolute_path + classifier_model
	print 'classifier_model',classifier_model
	n_hidden = int(sys.argv[2])
	do_active_fields_test = int(sys.argv[3])
...
#model
	...
	classifier_model = settings.absolute_path + "/tensor_model_sum/" + "model_5conv_GD512_h184_w184_c6_10_train_0.56-121_0.54-121.meta"
	#classifier_model = settings.absolute_path + "/testbench/6classes/" + "model_GD360_h184_w184_c6_6_3conv_L0.72_O1.0_R0.81_U0.75_V1.0_6g_0.88-9141.meta"
	#classifier_model = settings.absolute_path + "/testbench/6classes/" + "model_GD360_h184_w184_c6_3conv_L0.7_O1.0_U1.0_7_0.71-6381-mix-0.68g.meta"
	#classifier_model = settings.absolute_path + "/testbench/6classes/" + "model_GD360_h184_w184_c6all_0_tr_0.88-111.meta"
...
#dropout = [0.28, 0.4, 0.5, 0.5, 0.5, 0.5]  # 3,4,5,5
dropout = [0.95, 0.95, 0.5, 0.5, 1, 1]
...
# Data
LABEL_LIST = settings.absolute_path + '/FileList.txt'
LABEL_PATH = settings.data_label_path

# LABEL_LIST_TEST = '../FileList_TEST1_sing.txt'

LABEL_LIST_TEST = settings.absolute_path + '/FileList_TEST.txt'
...
if GENERATE_FILELIST == 1:
	...
	if TEST_with_Images or TEST_with_Video:
		tools.read_images(LABEL_LIST_TEST,random_read=False)
	else:
...
class Logger(object):
	def __init__(self):
		...
		self.log = open(datetime.now().strftime(settings.absolute_path + '/logs/log_eva_' + str_log + '.log'), "a")
...
def EVALUATE_IMAGES(session, num_class, img_list, _labels):
	...
	for i in range(0, TEST_length, 1):
		...
		if do_active_fields_test == 1:
			...
			positions = (
				(col) * slice_size, (row) * slice_size, (col + 1) * slice_size, (row + 1) * slice_size)  # x0,y0, x1,y1
...
def EVALUATE_IMAGES_mul(session, num_class, img_list, _labels):  # (eva)
	sess = session
	LABEL_LIST_TEST = img_list
	print 'evaluate with file list', LABEL_LIST_TEST
	LABELS = _labels
	n_classes = num_class
...
def EVALUATE_WITH_WEBCAM(camera_port, stop, num_class):  # (cam)
	...
	while True and not stop:  # hy: confirm camera is available
		...
		if frame is not None:
			...
			frame_crop_roi = frame_crop_color # frame_crop_roi contains learned roi if segmentation is done previously
			tools.get_tensor(sess,frame, frame_i,frame_crop_roi, frame_crop_color, crop_y1, crop_y2,
			                  crop_x1, crop_x2, n_classes,cvtcolor=True,screen_out=True)
...
def EVALUATE_VIDEO(VIDEO_FILE, num_class):  # (v)
	...
	while True:
		...
		if ret:
			...
			frame_crop_roi = frame_crop #frame_crop_roi is used together with seg
			#im_i, pre_tensor, n_classes, cvtcolor, screen_out
			tensorImgIn,test_labels = tools.get_tensor(video_frame_i, frame_crop_roi,n_classes, cvtcolor=True, screen_out=True)
			
			# send to classifier model
			RES = get_classify_result(sess, tensorImgIn, test_labels, video_frame_i, frame, frame_crop,
			                          crop_y1, crop_y2, crop_x1, crop_x2, screen_out=True)
			...
			confMat1_TEST,confMat2_TEST = tools.process_res(confMat1_TEST,confMat2_TEST,RES,frame_crop,
			                                          SAVE_CorrectClassified, SAVE_Misclassified,video_frame_i,target=target)
		...
		tools.do_statistics(confMat1_TEST, confMat2_TEST,video_frame_i)
...
if TEST_with_Video:
	with tf.Session() as sess:
		ckpt = tf.train.get_checkpoint_state(checkpoint_dir=settings.absolute_path + "/testbench/6classes/")
...
if TEST_with_Images:
	...
	with tf.Session() as sess:
		...
		ckpt = tf.train.get_checkpoint_state(checkpoint_dir= settings.absolute_path + "/tensor_model_sum/")  # "../backupModel/"
		print 'classifier2',classifier_model
		...
		#
		ckpt.model_checkpoint_path = classifier_model[:-5]
		#ckpt.model_checkpoint_path = settings.absolute_path +classifier_model[2:-5]
		...
		# LABELS = ['links/', 'rechts/']
		confMat1_TEST_i, count_labels, confMat3, class_probability, overall_acc, mAP = EVALUATE_IMAGES_mul(sess, 6,
		                                                                                                   eval_file_list,
		                                                                                                   LABELS)  #
		'''
		print '\nCount correctly classified'
		...
		if do_active_fields_test == 0 and do_multiple_test == True:
			#./testbench/6classes/1/model_GD360_h184_w184_c6all_16_tr_0.57-121.meta
		  n_bare = os.path.basename(classifier_model)[:-5]
		  print 'n_bare', n_bare
		  for filename in os.listdir(settings.absolute_path):#where all eva- txt previously saved
				#if filename.startswith('eva-' + n_bare + '.txt'):
			  if ('eva-'+n_bare) in filename:
					print 'found file'
					os.rename(settings.absolute_path + 'eva-' + n_bare + '.txt',
					          settings.absolute_path + 'eva-' + n_bare + '-acc' + str(overall_acc) + '-m' + str(mAP) + '.txt')
